...
uint8_t init_buffer[5];
uint8_t init = 1;
//uint8_t sent_msg[31] = "AT+SEND=110,16,Hello, Hunter.\r\n";
...
//	HAL_UART_Receive_IT(&huart2, buffer, sizeof(buffer));
	LoRa_init();
//	HAL_UART_Receive_DMA(&huart2, packet_buffer, sizeof(packet_buffer));
  /* USER CODE END 2 */
  ...
  /* USER CODE BEGIN WHILE */
    ...
    /* USER CODE BEGIN 3 */
		if (packet_buffer[0] == '+') {
			OLED_SendString(packet_buffer);
		} else if (packet_buffer[0] != 0) {
			OLED_SendString("WE GOT SOMETHING");
			OLED_SendString(packet_buffer);
		}
		else{
			OLED_SendString("nothing");
		}
		HAL_Delay(1000);
...
////		memset(packet_buffer, 0, sizeof(packet_buffer));
//		HAL_UART_Receive_DMA(&huart2, packet_buffer, sizeof(packet_buffer));
//	} else { //if we're using the initilazation buffer
//		while (strncmp(init_buffer, compare, 5));
//		memset(init_buffer, 0, sizeof(init_buffer));
//		HAL_UART_Receive_DMA(&huart2, init_buffer, sizeof(init_buffer));
//	}
